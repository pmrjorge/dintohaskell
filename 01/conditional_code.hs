module Main where
    printSmallNumber num = if num < 10 then print num else print "the number is too big!"
    main = printSmallNumber 3

    printSmallNumber' num = let msg = if num < 10 then show num else "the number is too big!"
                            in print msg

    sizeNumber num = if num < 3 then "that's a small number"
                     else 
                        if num < 10
    ...
    guardSize num 
        ...
        | otherwise = "that's a unfathomably big number"
